...
//by LX:
$("input[aria-label$='Выберите Модель техники из списка']").focus();
$(".wd-filter-wrapper .wd-swatches-filter").addClass('wd-filt-selected');
//$("input[aria-label$='Выберите Модель техники из списка']:visible").trigger("focus");
...
$('.shop-view.per-row-4').attr('href',grid_link);
      ...
      <?php  if(!empty($_GET['f_pa_length'])){
       ...
       if(empty($_GET['f_pa_width']) && empty($_GET['f_pa_height']))
        ...
        ?>
     xhtml=xhtml+'<div class=" woocommerce widget_layered_nav_filters"><ul class="asdfg"><li class="chosen chosen-brand chosen-brand-atlet"><a rel="nofollow" aria-label="Очистить фильтр" href="<?=$xlinkz?>">Длина <?=$_GET['f_pa_length']?></a></li></ul></div>'
   <?php  }?>
   ...
   <?php  if(!empty($_GET['f_pa_width'])){
       ...
       if(empty($_GET['f_pa_length']) && empty($_GET['f_pa_height']))
        $xlinkz=str_replace('filt=1','filt=', $xlinkz);
        ?>
     xhtml=xhtml+'<div class=" woocommerce widget_layered_nav_filters"><ul class="asdfg"><li class="chosen chosen-brand chosen-brand-atlet"><a rel="nofollow" aria-label="Очистить фильтр" href="<?=$xlinkz?>">Ширина <?=$_GET['f_pa_width']?></a></li></ul></div>'
   <?php  }?>
   ...
   <?php  if(!empty($_GET['f_pa_height'])){
       ...
       if(empty($_GET['f_pa_length']) && empty($_GET['f_pa_width']))
        $xlinkz=str_replace('filt=1','filt=', $xlinkz);
        ?>
     xhtml=xhtml+'<div class=" woocommerce widget_layered_nav_filters"><ul class="asdfg"><li class="chosen chosen-brand chosen-brand-atlet"><a rel="nofollow" aria-label="Очистить фильтр" href="<?=$xlinkz?>">Высота <?=$_GET['f_pa_height']?></a></li></ul></div>'
   <?php  }?>
